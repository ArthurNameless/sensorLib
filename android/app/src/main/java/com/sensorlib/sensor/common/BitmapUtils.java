...
public class BitmapUtils {
    private static String TAG = "BITMAP_UTILS";
    ...
    public static Bitmap scaleBitmap(Bitmap bitmap) {
        ...
        float minEdge = Math.min(w, h);
        float maxEdge = Math.max(w, h);
        float ratio = minEdge / maxEdge;
        int newW, newH;
        if (minEdge == w){
            newW = (int) ((float) 250.0 * ratio);
            newH = 250;
        } else {
            newH = (int) ((int) 250.0 * ratio);
            newW = 250;
        }
        if (w < 250 && h < 250){
            return bitmap;
        }
        Bitmap scaledBmp = Bitmap.createScaledBitmap(bitmap, newW, newH, true);
        return scaledBmp;
    }
}
