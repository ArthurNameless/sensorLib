...
import com.sensorlib.sensor.common.PreferenceUtils;
...
public class CameraSource {
    ...
    private static final String TAG = "AuraSense:CameraSource";
    ...
    public static SizePair selectSizePair(Camera camera, int desiredWidth, int desiredHeight) {
        ...
        Log.d(TAG, "Camera desired resolution is " + desiredHeight + "x" + desiredWidth);
        ...
        for (SizePair sizePair : validPreviewSizes) {
            ...
            float diff =
                    Math.abs(size.getWidth() - desiredWidth)
                            ...
                            + 5000 * Math.abs((float)desiredWidth / (float)desiredHeight - (float)size.getHeight() / (float)size.getWidth());
            Log.d(TAG, "Camera resolution " + size + " diff is " + diff);
            if (diff < minDiff) {
